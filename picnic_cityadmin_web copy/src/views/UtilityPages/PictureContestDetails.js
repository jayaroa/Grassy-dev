...
class AddEditComponent extends Component {
    ...
    axios
      ...
      .then(serverResponse => {
        ...
        if (!res.isError) {
            ...
            let pictureContestPicArr = [];
            pictureContestPicArr.push(this.state.mystery_picture);
            this.setState({ picture_contest_pictures: pictureContestPicArr });
        }
      });
  }
  ...
  addEditContest(action_type) {
    ...
    let dataToSend = {
      ...
      mystery_picture: that.state.mystery_picture,
      contest_type: "PICTURE",
...
//          }
          this.props.history.push("/picturecontests");
  ...
  savePictureImagePath = event => {
    ...
    if (event) {
      this._uploadImage(event, "contest").then(result => {
        ...
        let that = this;
        let pictureContestPicArr = [];
        pictureContestPicArr.push(park_image);
        this.setState({ mystery_picture : park_image });
        this.setState({ picture_contest_pictures: pictureContestPicArr });
      });
    }
  };
              ...
              <CardBody>
                <Row>
                  <Col md="10">
                    ...
                    {/* Park pictures */}
                    <Label htmlFor="mysteryPicture">Mystery Picture</Label><br/>
                    <Row>
                      {this.state.picture_contest_pictures.map(pic => {
                        return (
                          <Col md="3">
                            <Card>
                              <CardBody style={noPadding}>
                      ...
                      <Col md="2">
                        <Card style={addBtn}>
                          <Input
                            type="file"
                            style={fileInputStyle}
                            onChange={this.savePictureImagePath}
                          />
                          <CardBody>
                            <div
                              style={uploadBtnWrapper}
